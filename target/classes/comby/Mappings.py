...
tree1 = javalang.parse.parse_expression("a.b.random(foo, bar)")
tree2 = javalang.parse.parse_expression("UUID.random(foo.toString(), baz)")
...
def match_to_meta_pattern(source: str, target: str, renames) -> []:
    ...
    if len(result) > 0:
        for res in result:
            source = comby.rewrite(source=source, match=res['Before'], rewrite=res['After'], language=".java")
